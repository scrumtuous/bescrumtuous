...
The 'Scrum Pillars' don't constitute a specific category, but they frequently weave their way into answers about why a given event is taking place, or why a given Scrum artifacts is important.
...
As teams adapt, they must be open about how well the adaptation has worked, which requires the team to be transparent, which then leads to inspection and adaptation. It's a constant loop.
