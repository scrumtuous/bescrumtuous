...
The length of a Sprint is one month or less.  You'll be tested on that, so commit it to memory.
...
TIP: Some organizations declare a _Sprint Zero_ at the start of the project to get the team organized. This is not a Scrum endorsed concept. When mentioned on the exam, it is typically an incorrect option.
...
E is incorrect because Scrum Teams should be able to learn new technologies as required.
